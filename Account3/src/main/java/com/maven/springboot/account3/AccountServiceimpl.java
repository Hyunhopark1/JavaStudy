...
public class AccountServiceimpl implements AccountService{
    ...
    //1. accountRepository 인터페이스 선언
    private AccountRepository accountRepository;

    private List<Account> accountList = new ArrayList<>();

    //2. 생성자를 통해 BankApplicaition으로부터 String값 두개를 받아와 조건검사하여
    // 인터페이스 변수에 구현객체(JSON or File ) 대입
    public AccountServiceimpl(String fileType, String fileName) throws Exception{
        if ( "-j".equals(fileType) ) {
    ...
    //3. 인터페이스 메소드 호출
    public void loadData(List<Account> accountList) throws Exception {
        accountRepository.loadJson(accountList);
    }
    public void saveData(List<Account> accountList) throws Exception {
        accountRepository.saveJson(accountList);
    }
